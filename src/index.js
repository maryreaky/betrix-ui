...
/* WEBHOOK-TRACE-INSERTED */
// Lightweight webhook trace: always return 200 and log body for Telegram health checks.
// Remove this after you confirm webhook health.
try {
  if (typeof app === 'object' && app && app.post) {
    app.post('/webhook/telegram', (req, res) => {
      try {
        console.info('WEBHOOK-TRACE', { ts: Date.now(), body: req && req.body ? req.body : '<no-body>' });
      } catch (e) {
        console.error('WEBHOOK-TRACE-LOG-ERR', e && e.stack ? e.stack : e);
      }
      // Reply 200 fast so Telegram marks webhook healthy; actual processing should be handled by existing handlers or enqueued separately.
      res.status(200).send('ok');
    });
  } else {
    console.warn('WEBHOOK-TRACE: app.post not found - handler not mounted by this hotfix.');
  }
} catch (ex) {
  console.error('WEBHOOK-TRACE-FAILED', ex && ex.stack ? ex.stack : ex);
}
