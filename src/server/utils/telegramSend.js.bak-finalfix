/*
 Production telegramSend
 - Uses TELEGRAM_BOT_TOKEN from env
 - Normalizes common call shapes and posts to Telegram via HTTPS
...
async function sendText() {
  ...
  if (args.length === 1) {
    ...
    aiResp = extractText(args[1]) ? args[1] : args[0];
  ...
  if (!aiResp) { for (const a of args) { const t = extractText(a); if (t && t.length) { aiResp = a; break } } }
